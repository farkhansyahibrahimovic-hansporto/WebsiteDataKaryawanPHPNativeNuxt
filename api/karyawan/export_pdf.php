...
/**
 * ======================================================================
 *  API ENDPOINT: EXPORT DATA KARYAWAN KE PDF
 * ======================================================================
 */
...
try {
    ...
    error_log("Export PDF - Filters: " . json_encode($filters));
    
    // Ambil semua data karyawan
    $dataKaryawan = ambilListKaryawan($filters);
    
    error_log("Export PDF - Total data: " . count($dataKaryawan));
    ...
    if (empty($dataKaryawan)) {
        ...
        logExport('PDF', 'data_karyawan_' . date('Ymd_His') . '.pdf', 0, $filters, false, "Tidak ada data untuk diexport");
        echo json_encode($res);
        exit;
    }
    ...
    // Generate filename
    $filename = 'data_karyawan_' . date('Ymd_His') . '.pdf';
    
    // Log export
    logExport('PDF', $filename, count($dataKaryawan), $filters, true);
    
    // CRITICAL: Jangan bersihkan buffer atau remove headers lagi
    // Fungsi generatePDF() akan handle semuanya
    
    // Generate & output PDF
    generatePDF($dataKaryawan, $filename);
    // Fungsi generatePDF sudah exit, kode di bawah tidak akan dijalankan
    ...
    $res = jsonError("Gagal export PDF: " . $e->getMessage(), null, 500);
    tulisLogDB($pdo, $endpoint, $method, $_GET, $res);
    
    $filename = 'data_karyawan_' . date('Ymd_His') . '.pdf';
    logExport('PDF', $filename, 0, $filters ?? [], false, $e->getMessage());
